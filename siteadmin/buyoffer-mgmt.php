...
$sqlPgn = "SELECT count(*) as cnt FROM tbl_buyoffer $keysearch";
...
$sqlMgmt = "SELECT * FROM tbl_buyoffer $keysearch ORDER BY fld_product_name ASC LIMIT $startPage, $RecsToShow";
$rsMgmt = $dbquery->query($sqlMgmt) or die('Error: ' . $dbquery->error);
  ...
  document.getElementById("search").focus();
	...
	<section class="middlepart">
		<div class="right-panel">
			...
			<div class="addfiles"><a href="buyoffer.php?companyid=<?php echo $companyid;?>&pageid=<?php echo $pageid; ?>"><i class="fa fa-plus-circle white"></i>Add New Buy Offer</a></div>
					<div class="clr"></div>
						...
						<table class="table table-bordered table-hover">
							...
							if($numMgmt > 0)
							{
								while($rowMgmt = $rsMgmt->fetch_object())
								{
									...
									$date = date("d M Y h:i:s",strtotime($rowMgmt->fld_added_on));
									...
									{
										$sts='Inactive';
										$showsts = '<a href="buyoffer-del.php?companyid='.$companyid.'&productid='.$productid.'&pageid='.$pageid.'&status='.$sts.'&task=chk" title="active" class="activeicon"><i class="fa fa-check-circle green"></i></a>';
									}
									else
									{
										$sts='Active';
										$showsts = '<a href="buyoffer-del.php?companyid='.$companyid.'&productid='.$productid.'&pageid='.$pageid.'&status='.$sts.'&task=chk" title="inactive" class="inactive"><i class="fa fa-times-circle red"></i></a>';
									}
							     ...
							     <a class="edit-table" data-toggle="tooltip" data-original-title="Edit" title="edit" href="buyoffer.php?companyid=<?php echo $companyid; ?>&productid=<?php echo $productid; ?>&pageid=<?php echo $pageid; ?>"><i class="fa fa-pencil"></i></a>	
										<a href="buyoffer-del.php?companyid=<?php echo $companyid; ?>&productid=<?php echo $productid;?>&pageid=<?php echo $pageid; ?>&task=del" onclick="return confirmDelete();" class="edit-table" title="delete"> <span id="<?=$sr?>"  data-toggle="tooltip" data-placement="left" title="Delete"><i class="fa fa-trash"></i></span></a>
									</td>
								</tr>
								<?php
								$sr++;
