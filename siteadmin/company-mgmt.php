...
$sqlPgn = "SELECT count(*) as cnt FROM tbl_company $keysearch";
...
$sqlMgmt = "SELECT * FROM tbl_company $keysearch ORDER BY fld_id DESC LIMIT $startPage, $RecsToShow";
		...
		<div class="right-panel">
			...
			<?php if($successmsg!="") { ?><div id="categoryDeleted" class="alert alert-success" style="text-align:center;"><div <?php echo $headingcls; ?>><?php echo $successmsg; ?></div></div><?php } ?>
				...
				<li><input type="text" name="keywordsearch" class="search" value="<?php echo $keywordsearch; ?>" autofocus /></li>
					...
					<div class="addfiles"><a href="company.php?pageid=<?php echo $pageid; ?>"><i class="fa fa-plus-circle white"></i>Add New Company</a></div>
					<div class="clr"></div>
						...
						<table class="table table-bordered table-hover">
							<thead>
								<tr class="th-heading">
                                    ...
                                    <th width="10%">Company Image</th>
                                  
									<th width="9%">Tax Number</th>
									<th width="8%">Buisness Model</th>
							...
							if($numMgmt > 0)
							{
								while($rowMgmt = $rsMgmt->fetch_object())
								{
							      ...
							      $sql = "SELECT fld_id, fld_company_name FROM tbl_exp_section where fld_id='".$china_exp_name."'";
                           ...
                           <!-- <a  title="Manage Services" href="service-mgmt.php?companyid=<?php echo $companyid; ?>" class="edit-table"><img src="images/detail.png" /></a> -->
                              ...
                              <?php /*?><a  title="Buy Offer" href="buyoffer-mgmt.php?companyid=<?php echo $companyid; ?>" class="edit-table"><img src="images/detail.png" /></a><?php */?>
                              
                               <a  title="Manage Documents" href="image-mgmt.php?companyid=<?php echo $companyid; ?>" class="edit-table"><img src="images/detail.png" /></a> 
							
                             <a  title="More Images" href="company_addon_image_mngmt.php?companyid=<?php echo $companyid; ?>" class="edit-table"><img src="images/detail.png" /></a>
                             	
							     <a class="edit-table" data-toggle="tooltip" data-original-title="Edit" title="edit" href="company.php?companyid=<?php echo $companyid; ?>&pageid=<?php echo $pageid; ?>"><i class="fa fa-pencil"></i></a>	
										<a href="company-del.php?companyid=<?php echo $companyid; ?>&pageid=<?php echo $pageid; ?>&task=del" onclick="return confirmDelete();" class="edit-table" title="delete"> <span id="<?=$sr?>"  data-toggle="tooltip" data-placement="left" title="Delete"><i class="fa fa-trash"></i></span></a>
									</td>
								</tr>
								<?php
								$sr++;
