 ...
 if($email!="")
{
		...
		if($companyid > 0)
		{
			$sqlUpdate = "UPDATE tbl_exp_section set  
			       fld_email ='".$email."',
				   fld_name = '".$name."',
		...
		else
		{
			       ...
			       $sqlInsert = "INSERT INTO tbl_exp_section SET
			       fld_email ='".$email."',
		 ...
		 if($_FILES['fld_image']['name']!='')
		      {	
 				$sqlimage = "SELECT fld_image FROM tbl_exp_section where fld_id='$companyid'";
				...
				if($file)
				{
					...
					if($companyid > 0)
					{
						$sqlUpdate = "UPDATE  tbl_exp_section set fld_image='$fileName' where fld_id='$companyid'";
						$rsUpdate = $dbquery->query($sqlUpdate) or die("Update Error:" . $dbquery->error);
						$updts = 2;
					}
					else
					{
						...
						$sqlInsert = "UPDATE tbl_exp_section set fld_image='$fileName' where fld_id='$compid'";
						$rsInsert = $dbquery->query($sqlInsert) or die("Insert Error:" .$dbquery->error);
...
##Show Detail
$sqlDisp = " SELECT * FROM tbl_exp_section WHERE fld_id='$companyid'";
	...
	<section class="middlepart">
		...
		<div class="right-panel">
			<aside class="rightsection iframe dboard">
				<h2><?php if($companyid=="") { echo "Add"; } else { echo "Edit"; } ?> China Exp Section <?php if($messageText!="") { ?>
				...
				<input type="hidden" name="pageid" value="<?php echo $pageid; ?>" />
					<ul>
						...
						<input type="text" name="company_name" id="company_name" value="<?php echo $rowDisp->fld_company_name;?>" size="60" placeholder="China Exp name:" required /> 
						</li>
