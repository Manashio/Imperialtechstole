...
if($msgupd==1) { $successmsg = "Category successfully Added!"; }
elseif($msgupd==2) { $successmsg = "Category successfully edited!"; }
elseif($msgupd==3) { $successmsg = "Category status successfully changed!"; }
elseif($msgupd==4) { $successmsg = "Category deleted Successfully!"; }
...
$sqlMgmt = "SELECT * FROM tbl_category $keysearch ORDER BY fld_name ASC,fld_name ASC LIMIT $startPage, $RecsToShow";
						...
						<table class="table table-bordered table-hover">
							...
							if($numMgmt > 0)
							{
								while($rowMgmt = $rsMgmt->fetch_object())
								{
									...
									{
										$sts='Inactive';
										$showsts = '<a href="category-del.php?catid='.$catid.'&pageid='.$pageid.'&status='.$sts.'&task=chk" title="active" class="activeicon"><i class="fa fa-check-circle green"></i></a>';
									}
									else
									{
										$sts='Active';
										$showsts = '<a href="category-del.php?catid='.$catid.'&pageid='.$pageid.'&status='.$sts.'&task=chk" title="inactive" class="inactive"><i class="fa fa-times-circle red"></i></a>';
									}
...
-->
							...
							<a class="edit-table" data-toggle="tooltip" data-original-title="Edit" title="edit" href="category.php?catid=<?php echo $catid; ?>&pageid=<?php echo $pageid; ?>"><i class="fa fa-pencil"></i></a>	
										<a href="category-del.php?catid=<?php echo $catid; ?>&pageid=<?php echo $pageid; ?>&task=del" onclick="return confirmDelete();" class="edit-table" title="delete"> <span id="<?=$sr?>"  data-toggle="tooltip" data-placement="left" title="Delete"><i class="fa fa-trash"></i></span></a>
									</td>
								</tr>
								<?php
								$sr++;
