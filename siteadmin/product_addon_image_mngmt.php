...
$Pagename = "Product Image Management";
...
if($msgupd==1) { $successmsg = "Product Image successfully Added!"; }
elseif($msgupd==2) { $successmsg = "Product Image successfully edited!"; }
elseif($msgupd==3) { $successmsg = "Product Image status successfully changed!"; }
elseif($msgupd==4) { $successmsg = "Product Image deleted Successfully!"; }
...
$sqlPgn = "SELECT count(*) as cnt FROM tbl_product_images where fld_company_id='".$productid."'";
...
$sqlMgmt = "SELECT * FROM tbl_product_images where fld_company_id='".$productid."' ORDER BY fld_id ASC LIMIT $startPage, $RecsToShow";
		...
		<div class="right-panel">
			...
			<?php if($successmsg!="") { ?><div id="categoryDeleted" class="alert alert-success" style="text-align:center;"><div <?php echo $headingcls; ?>><?php echo $successmsg; ?></div></div><?php } ?>
				...
				<li><input type="text" name="keywordsearch" class="search" value="<?php echo $keywordsearch; ?>" autofocus /></li>
					...
					<div class="addfiles"><a href="product_addon_images.php?productid=<?php echo $_GET['productid']; ?>"><i class="fa fa-plus-circle white"></i>Add New</a></div>
					<div class="clr"></div>
						...
						<table class="table table-bordered table-hover">
							...
							if($numMgmt > 0)
							{
								while($rowMgmt = $rsMgmt->fetch_object())
								{
									...
									{
										$sts='Inactive';
										$showsts = '<a href="product-image-del.php?sldid='.$sldid.'&pageid='.$pageid.'&status='.$sts.'&task=chk" title="active" class="activeicon"><i class="fa fa-check-circle green"></i></a>';
									}
									else
									{
										$sts='Active';
										$showsts = '<a href="product-image-del.php?sldid='.$sldid.'&pageid='.$pageid.'&status='.$sts.'&task=chk" title="inactive" class="inactive"><i class="fa fa-times-circle red"></i></a>';
									}
								...
								<!--	<td><?php echo $arrayImgcomp[$fld_image_cat]; ?></td>-->
									...
									<a href="product-image-del.php?productid=<?php echo $productid; ?>&id=<?php echo $id; ?>&task=del" onclick="return confirmDelete();" class="edit-table" title="delete"> <span id="<?=$sr?>"  data-toggle="tooltip" data-placement="left" title="Delete"><i class="fa fa-trash"></i></span></a>
									</td>
								</tr>
								<?php
								$sr++;
