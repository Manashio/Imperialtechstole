...
$product_model	= $dbquery->real_escape_string($_REQUEST['product_model']);
$product_price	= $dbquery->real_escape_string($_REQUEST['product_price']);
$product_category	= $dbquery->real_escape_string($_REQUEST['product_category']);
...
{$product_code = "HS".rand(11,99).rand(111,999).rand(5555,8888);}


$sqlseo = "SELECT * FROM tbl_product WHERE fld_product_name='$product_name'";
...
if($product_name!="")
{
		...
		if($productid > 0)
		{
			      $sqlUpdate = "UPDATE tbl_product set  
				   ...
				   fld_subcatid = '".$subcatid."',
				    fld_subsubcatid = '".$subsubcatid."',
				   fld_product_name ='".$product_name."',
				    fld_product_code ='".$product_code."',
				    fld_product_model ='".$product_model."',
					 fld_product_category ='".$product_category."',
		...
		else
		{
			       ...
			       $sqlInsert = "INSERT INTO tbl_product SET
				   fld_company_id = '".$companyid."',
				    fld_catid = '".$catid."',
				   fld_subcatid = '".$subcatid."',
				   fld_subsubcatid = '".$subsubcatid."',
			...
			$updts = 1;
		      ...
		      {	
				...
				if($file)
				{
					...
					if($productid > 0)
					{
						$sqlUpdate = "UPDATE  tbl_product set fld_image='$fileName' where fld_id='$productid'";
						$rsUpdate = $dbquery->query($sqlUpdate) or die("Update Error:" . $dbquery->error);
						$updts = 2;
					}
					else
					{
						...
						$sqlInsert = "UPDATE tbl_product set fld_image='$fileName' where fld_id='$prodid'";
						$rsInsert = $dbquery->query($sqlInsert) or die("Insert Error:" .$dbquery->error);
	  ...
	  if($updts > 0)
		{
				...
				echo "<script>document.location.href='product-mgmt.php?companyid=".$companyid."&pageid=".$pageid."&msgupd=".$updts."';</script>";
				exit;
...
##Show Detail
$sqlDisp = " SELECT * FROM tbl_product WHERE fld_id='$productid'";
...
{	 
	...
	var subcategory = $("#subcategory").val();
	 ...
	 $.ajax({
			...
			url: "ajaxsubsubcategory.php", 
			success: function(result){
				 $("#resultsubsubcat").html(result);
				 var chk = $.trim(result);
...
<section class="middlepart">
  ...
  <div class="right-panel">
    <aside class="rightsection iframe dboard">
      <h2>
        <?php if($productid=="") { echo "Add"; } else { echo "Edit"; } ?>
        Product
        <?php if($messageText!="") { ?>
        <div class="errormsg"><?php echo $messageText; ?></div>
        <?php } ?>
      </h2>
          ...
          <ul>
            ...
            <li>
              ...
              <select name="category" id="category" onchange="return subCategory();" required>
                ...
                <?php $sqlcat = "SELECT * FROM tbl_category where fld_parentid='0' and fld_status='Active' order by fld_name";
            ...
            <li id="resultsubcat">
              ...
              <select name="subcategory" id="subcategory" onchange="return subsubCategory();"  >
            ...
            <li id="resultsubsubcat">
              <?php if($productid!="") {  ?>
              <label>Subcategory Name:</label>
              <select name="subsubcategory" id="subsubcategory"  >
                <option value="">Select Subcategory</option>
                <?php $sqlcat1 = "SELECT * FROM tbl_category where fld_parentid='$rowDisp->fld_subcatid' and fld_status='Active'";
                              $rsCat1 = $dbquery->query($sqlcat1);
                              while($rowCat1 = $rsCat1->fetch_object())
                              {
                            ?>
                <option value="<?php echo $rowCat1->fld_id; ?>"  <?php echo ($rowDisp->fld_subsubcatid==$rowCat1->fld_id) ? "selected" : "";?> ><?php echo $rowCat1->fld_name; ?> </option>
                <?php }?>
              </select>
              <?php }?>
            </li>
            <li>
              ...
              <input type="text" name="product_name" id="product_name" value="<?php echo $rowDisp->fld_product_name;?>" size="60" placeholder="Product Name" required />
              <span id="emailerror"></span> </li>
            <li>
            <li>
              <label>Product Code:</label>
              <input type="text" name="product_code" id="product_code" value="<?php echo $rowDisp->fld_product_code;?>" size="60" placeholder="Product Code" />
              <span id="emailerror"></span> </li>
            <li>
              <label>Product Model:</label>
              <input type="text" name="product_model" id="product_model" value="<?php echo $rowDisp->fld_product_model;?>" size="60" placeholder="Product Model" required />
              <span id="emailerror"></span> </li>
            <li>
              <label>Product Category:</label>
              <input type="text" name="product_category" id="product_category" value="<?php echo $rowDisp->fld_product_category;?>" size="60" placeholder="Product Category" required />
              <span id="emailerror"></span> </li>
            <li>
              <label>Product Price/wholesale price:</label>
              <input type="text" name="product_price" id="product_price" value="<?php echo $rowDisp->fld_product_price;?>" size="60" placeholder="Product Price" required />
              <span id="emailerror"></span> </li>
            <li style="display:none;">
              ...
              <?php if($productid!="") {  ?>
              <img height="100" width="100" src="../images/product/<?php echo $rowDisp->fld_image;?>" /> <br />
              <br />
              <?php }?>
            </li>
            <li>
