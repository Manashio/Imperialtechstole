...
$Pagename = "China Exp Section Management";
...
if($msgupd==1) { $successmsg = "Company successfully Added!"; }
elseif($msgupd==2) { $successmsg = "Company successfully edited!"; }
elseif($msgupd==3) { $successmsg = "Company status successfully changed!"; }
elseif($msgupd==4) { $successmsg = "Company deleted Successfully!"; }
...
$sqlPgn = "SELECT count(*) as cnt FROM tbl_exp_section $keysearch";
...
$sqlMgmt = "SELECT * FROM tbl_exp_section $keysearch ORDER BY fld_id DESC LIMIT $startPage, $RecsToShow";
		...
		<div class="right-panel">
			...
			<?php if($successmsg!="") { ?><div id="categoryDeleted" class="alert alert-success" style="text-align:center;"><div <?php echo $headingcls; ?>><?php echo $successmsg; ?></div></div><?php } ?>
				...
				<li><input type="text" name="keywordsearch" class="search" value="<?php echo $keywordsearch; ?>" autofocus /></li>
					...
					<div class="addfiles"><a href="exp.php?pageid=<?php echo $pageid; ?>"><i class="fa fa-plus-circle white"></i>Add new exp section</a></div>
					<div class="clr"></div>
						...
						<table class="table table-bordered table-hover">
							<thead>
								<tr class="th-heading">
									...
									<th width="10%">China Exp name</th>
									<th width="11%">Contact Person</th>
                                    <th width="10%">Exp section logo</th>
							...
							if($numMgmt > 0)
							{
								while($rowMgmt = $rsMgmt->fetch_object())
								{
									...
									{
										$sts='Inactive';
										$showsts = '<a href="exp-del.php?companyid='.$companyid.'&pageid='.$pageid.'&status='.$sts.'&task=chk" title="active" class="activeicon"><i class="fa fa-check-circle green"></i></a>';
									}
									else
									{
										$sts='Active';
										$showsts = '<a href="exp-del.php?companyid='.$companyid.'&pageid='.$pageid.'&status='.$sts.'&task=chk" title="inactive" class="inactive"><i class="fa fa-times-circle red"></i></a>';
									}
 								...
 								<tr>
                                    ...
                                    <td><?php echo $arrProChina[$stateid]; ?></td>
									<td><?php echo $date; ?> </td>
									
									<td class="text-center"><?php echo $showsts; ?></td>
									<td class="text-center">
                         <!--   <a  title="Manage Prdoucts" href="product-mgmt.php?companyid=<?php echo $companyid; ?>" class="edit-table"><img src="images/detail.png" /></a> 
                           <a  title="Manage Services" href="service-mgmt.php?companyid=<?php echo $companyid; ?>" class="edit-table"><img src="images/detail.png" /></a> 
                           
                              <a  title="Buy Offer" href="buyoffer-mgmt.php?companyid=<?php echo $companyid; ?>" class="edit-table"><img src="images/detail.png" /></a>
                              
                               <a  title="Sell Offer" href="selloffer-mgmt.php?companyid=<?php echo $companyid; ?>" class="edit-table"><img src="images/detail.png" /></a> 
								-->
							     <a class="edit-table" data-toggle="tooltip" data-original-title="Edit" title="edit" href="exp.php?companyid=<?php echo $companyid; ?>&pageid=<?php echo $pageid; ?>"><i class="fa fa-pencil"></i></a>	
										<a href="exp-del.php?companyid=<?php echo $companyid; ?>&pageid=<?php echo $pageid; ?>&task=del" onclick="return confirmDelete();" class="edit-table" title="delete"> <span id="<?=$sr?>"  data-toggle="tooltip" data-placement="left" title="Delete"><i class="fa fa-trash"></i></span></a>
									</td>
								</tr>
								<?php
								$sr++;
