...
$expire  = date("Y-m-d", strtotime($_REQUEST['expire']));
$product_code = rand(1,9).rand(11,99).rand(111,999).rand(5555,8888);

$sqlseo = "SELECT * FROM tbl_selloffer WHERE fld_product_name='$product_name'";
...
if($product_name!="")
{
		...
		if($productid > 0)
		{
			      $sqlUpdate = "UPDATE tbl_selloffer set  
		...
		else
		{
			       ...
			       $sqlInsert = "INSERT INTO tbl_selloffer SET
				   fld_company_id = '".$companyid."',
				    fld_catid = '".$catid."',
				   fld_subcatid = '".$subcatid."',
			...
			$updts = 1;
		      ...
		      {	
				...
				if($file)
				{
					...
					if($productid > 0)
					{
						$sqlUpdate = "UPDATE  tbl_selloffer set fld_image='$fileName' where fld_id='$productid'";
						$rsUpdate = $dbquery->query($sqlUpdate) or die("Update Error:" . $dbquery->error);
						$updts = 2;
					}
					else
					{
						...
						$sqlInsert = "UPDATE tbl_selloffer set fld_image='$fileName' where fld_id='$prodid'";
						$rsInsert = $dbquery->query($sqlInsert) or die("Insert Error:" .$dbquery->error);
	  ...
	  if($updts > 0)
		{
				...
				echo "<script>document.location.href='selloffer-mgmt.php?companyid=".$companyid."&pageid=".$pageid."&msgupd=".$updts."';</script>";
				exit;
...
##Show Detail
$sqlDisp = " SELECT * FROM tbl_selloffer WHERE fld_id='$productid'";
	...
	<section class="middlepart">
		...
		<div class="right-panel">
			<aside class="rightsection iframe dboard">
				<h2><?php if($productid=="") { echo "Add"; } else { echo "Edit"; } ?> Selloffer <?php if($messageText!="") { ?>
				...
				<input type="hidden" name="pageid" value="<?php echo $pageid; ?>" />
					<ul>
                        ...
                        <label>Subcategory Name:</label>
						 ...
						 <li>
						<label>Title:</label>
						<input type="text" name="product_name" id="product_name" value="<?php echo $rowDisp->fld_product_name;?>" size="60" placeholder="Title" required /> 
                      
						</li>
                        <li>
                        <label>Offer Expirt Date:</label>
						<input type="text" name="expire" id="expire" value="<?php echo date("d-m-Y", strtotime($rowDisp->fld_expire));?>" size="60" placeholder="Expir Date"  readonly="readonly" />                       </li>
                        <li>
							<label> Image:</label>
							...
							<?php if($productid!="") {  ?>
							<img height="100" width="100" src="../images/product/thumb/<?php echo $rowDisp->fld_image;?>" />
								<br />
								<br />
								<?php }?>
