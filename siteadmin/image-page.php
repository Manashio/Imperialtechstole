 ...
 if(isset($_REQUEST['submit']))
 {
	 ...
	 $image_cat = $dbquery->real_escape_string($_REQUEST['image_cat']);
       ...
       move_uploaded_file($fileTmp,"../images/document/".$namefile);
		...
		$sqlInsert = "INSERT INTO tbl_document SET fld_image='".$namefile."',fld_company_id='".$company_id."',fld_image_cat = '".$image_cat."'";
		...
		if($updts > 0)
		{
				...
				echo "<script>document.location.href='image-mgmt.php?msgupd=".$updts."&companyid=".$company_id."';</script>";
				exit;
...
##Show Detail
$sqlDisp = " SELECT * FROM tbl_document WHERE fld_id='$sldid'";
	...
	<section class="middlepart">
		...
		<div class="right-panel">
			<aside class="rightsection iframe dboard">
				<h2><?php if($sldid=="") { echo "Add"; } else { echo "Edit"; } ?> Document Image <?php if($messageText!="") { ?><div class="errormsg"><?php echo $messageText; ?></div><?php } ?></h2>
					...
					<ul>
						...
						<label>Select Image Category:</label>
						 ...
						 <select  name="image_cat" id="image_cat" required>
                          ...
                          <?php foreach($arrayImgcat as $key=>$val){?>
