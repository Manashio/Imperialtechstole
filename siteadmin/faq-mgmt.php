...
if($msgupd==1) { $successmsg = "Faq successfully Added!"; }
elseif($msgupd==2) { $successmsg = "Faq successfully edited!"; }
elseif($msgupd==3) { $successmsg = "Faq status successfully changed!"; }
elseif($msgupd==4) { $successmsg = "Faq deleted Successfully!"; }

##Search Keyword
$keysearch = " WHERE (fld_id!='0')";

if($keywordsearch!="")
{
	$keysearch .= " AND (fld_title LIKE '%$keywordsearch%' OR fld_title='$keywordsearch')";
}
...
$sqlPgn = "SELECT count(*) as cnt FROM tbl_faq $keysearch";
...
$sqlMgmt = "SELECT * FROM tbl_faq $keysearch ORDER BY fld_id DESC LIMIT $startPage, $RecsToShow";
		...
		<div class="right-panel">
			...
			<?php if($successmsg!="") { ?><div id="categoryDeleted" class="alert alert-success" style="text-align:center;"><div <?php echo $headingcls; ?>><?php echo $successmsg; ?></div></div><?php } ?>
				...
				<li><input type="text" name="keywordsearch" class="search" value="<?php echo $keywordsearch; ?>" autofocus /></li>
					...
					<div class="addfiles"><a href="faq.php?pageid=<?php echo $pageid; ?>"><i class="fa fa-plus-circle white"></i>Add New faq</a></div>
					<div class="clr"></div>
						...
						<table class="table table-bordered table-hover">
							...
							if($numMgmt > 0)
							{
								while($rowMgmt = $rsMgmt->fetch_object())
								{
									...
									$desc = $rowMgmt->fld_desc;
 									$status = $rowMgmt->fld_status;
																											
									if($status=='Active')
									{
										$sts='Inactive';
										$showsts = '<a href="faq-del.php?catid='.$catid.'&pageid='.$pageid.'&status='.$sts.'&task=chk" title="active" class="activeicon"><i class="fa fa-check-circle green"></i></a>';
									}
									else
									{
										$sts='Active';
										$showsts = '<a href="faq-del.php?catid='.$catid.'&pageid='.$pageid.'&status='.$sts.'&task=chk" title="inactive" class="inactive"><i class="fa fa-times-circle red"></i></a>';
									}
								...
								<a class="edit-table" data-toggle="tooltip" data-original-title="Sub Category" title="Sub Category" href="subcategory-mgmt.php?parentid=<?php echo $catid; ?>&pageid=<?php echo $pageid; ?>"><i class="fa fa-list"></i></a>	
								
							<a class="edit-table" data-toggle="tooltip" data-original-title="Edit" title="edit" href="faq.php?catid=<?php echo $catid; ?>&pageid=<?php echo $pageid; ?>"><i class="fa fa-pencil"></i></a>	
										<a href="faq-del.php?catid=<?php echo $catid; ?>&pageid=<?php echo $pageid; ?>&task=del" onclick="return confirmDelete();" class="edit-table" title="delete"> <span id="<?=$sr?>"  data-toggle="tooltip" data-placement="left" title="Delete"><i class="fa fa-trash"></i></span></a>
									</td>
								</tr>
								<?php
								$sr++;
