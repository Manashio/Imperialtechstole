...
if($productid > 0)
{
	if($task=="chk")
	{
		$sqlsts = "UPDATE tbl_buyoffer set fld_status='$status' where fld_id='$productid'";
	...
	elseif($task=="del")
	{
		...
		$sqlDel = "DELETE FROM tbl_buyoffer where fld_id='".$productid."'";
		$rsDel = $dbquery->query($sqlDel) or die("Delete Error:" . $dbquery->error);

		$updts = 4;
	}
}
echo "<script>document.location.href='buyoffer-mgmt.php?companyid=".$companyid."&msgupd=".$updts."&pageid=".$pageid."';</script>";
exit;
