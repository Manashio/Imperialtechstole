...
if($msgupd==1) { $successmsg = "Membership successfully Added!"; }
elseif($msgupd==2) { $successmsg = "Membership successfully edited!"; }
elseif($msgupd==3) { $successmsg = "Membership status successfully changed!"; }
elseif($msgupd==4) { $successmsg = "Membership deleted Successfully!"; }
...
$sqlPgn = "SELECT count(*) as cnt FROM tbl_membership $keysearch";
...
$sqlMgmt = "SELECT * FROM tbl_membership $keysearch ORDER BY fld_date DESC LIMIT $startPage, $RecsToShow";
		...
		<div class="right-panel">
			...
			<?php if($successmsg!="") { ?><div id="categoryDeleted" class="alert alert-success" style="text-align:center;"><div <?php echo $headingcls; ?>><?php echo $successmsg; ?></div></div><?php } ?>
				...
				<li><input type="text" name="keywordsearch" class="search" value="<?php echo $keywordsearch; ?>" autofocus /></li>
					...
					<!--<div class="addfiles"><a href="membership.php?pageid=<?php echo $pageid; ?>"><i class="fa fa-plus-circle white"></i>Add Membership </a></div>-->
					<div class="clr"></div>
					...
					<div class="tablebox table-responsive">
						<table class="table table-bordered table-hover">
							<thead>
								<tr class="th-heading">
									<th width="5%">S. No</th>
									<th width="22%">Membership Name</th>
									<!--<th width="22%">Date</th>-->
							...
							if($numMgmt > 0)
							{
								while($rowMgmt = $rsMgmt->fetch_object())
								{
									...
									{
										$sts='Inactive';
										$showsts = '<a href="membership-del.php?articlesid='.$artilceid.'&pageid='.$pageid.'&status='.$sts.'&task=chk" title="active" class="activeicon"><i class="fa fa-check-circle green"></i></a>';
									}
									else
									{
										$sts='Active';
										$showsts = '<a href="membership-del.php?articlesid='.$artilceid.'&pageid='.$pageid.'&status='.$sts.'&task=chk" title="inactive" class="inactive"><i class="fa fa-times-circle red"></i></a>';
									}
							...
							<a class="edit-table" data-toggle="tooltip" data-original-title="Edit" title="edit" href="membership.php?articlesid=<?php echo $artilceid; ?>&pageid=<?php echo $pageid; ?>"><i class="fa fa-pencil"></i></a>	
										<!--<a href="membership-del.php?articlesid=<?php echo $artilceid; ?>&pageid=<?php echo $pageid; ?>&task=del" onclick="return confirmDelete();" class="edit-table" title="delete"> <span id="<?=$sr?>"  data-toggle="tooltip" data-placement="left" title="Delete"><i class="fa fa-trash"></i></span></a>	-->							</td>
								</tr>
								<?php
								$sr++;
									}
